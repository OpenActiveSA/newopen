...
// List of Feather icons we're using
const featherIcons = [
  'lock', 'mail', 'heart', 'star', 'calendar', 'clock', 
  'user', 'map-pin', 'phone', 'settings', 'search', 'play'
];

// SVG templates for each icon (simplified versions)
const iconSVGs = {
  ...
  'play': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  <polygon points="5,3 19,12 5,21"></polygon>
</svg>`
};

// Create the feather-icons directory
const featherDir = path.join(__dirname, '..', 'src', 'assets', 'feather-icons');
if (!fs.existsSync(featherDir)) {
  fs.mkdirSync(featherDir, { recursive: true });
}

// Download each icon
featherIcons.forEach(iconName => {
  const svgContent = iconSVGs[iconName];
  if (svgContent) {
    const fileName = `${iconName}.svg`;
    const filePath = path.join(featherDir, fileName);
    fs.writeFileSync(filePath, svgContent);
    console.log(`Downloaded: ${fileName}`);
  }
});

console.log(`\n✅ Downloaded ${featherIcons.length} Feather icons to: ${featherDir}`);
console.log('You can now use these SVG files directly in your project!');
